    ...
    <form method="POST" >
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <?php $getsegment = Request::segment(3); ?>
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody> <?php $srno = 0; ?>
                            @foreach ($leads as $list)
                                <?php
                                $sercount =  App\Models\ServiceTaken::select()->where('leadid',$list['id'])->sum('payment');
                                $paycount =  App\Models\PaymentTaken::select()->where('leadid',$list['id'])->sum('payment');
                                $spt = $sercount;
                                $pt = $sercount-$paycount;
                                ?>
                                <tr>
                                    <td><?= ++$srno ?></td>
                                    <td><div class="btn-group"><?php
                                        ...
                                        <?php }else if($getsegment == "a_process"){ ?>
                                           <a href="{{url('/admin/mylead/editform/'.Crypt::encrypt($list['id']))}}" type="button" class="btn btn-default"><i class="fa fa-edit"></i> </a>
                                           <a href="{{url('/admin/mylead/print/'.Crypt::encrypt($list['id']))}}" type="button" class="btn btn-info"><i class="fa fa-print"></i> </a>
                                           <a href="{{url('/admin/mylead/service/'.Crypt::encrypt($list['id']))}}" type="button" class="btn btn-warning"><i class="fa fa-plus"></i> </a>
                                           <a href="{{url('/admin/mylead/detail/'.Crypt::encrypt($list['id'])).'?update=remark'}}" type="button" class="btn btn-success"><i class="fa fa-eye"></i></a>
                                        <?php }else if($getsegment == "a_complete"){ ?>
                                            <a href="{{url('/admin/mylead/print/'.Crypt::encrypt($list['id']))}}" type="button" class="btn btn-info"><i class="fa fa-print"></i> </a>
                                            <a href="{{url('/admin/mylead/detail/'.Crypt::encrypt($list['id'])).'?update=remark'}}" type="button" class="btn btn-success"><i class="fa fa-eye"></i></a>
                                        <?php } ?></div></td>
