            ...
            <div class="row">
                {{-- <div class="col-md-3">
                    <div class="form-group">
                        <label for="date_from">Search By</label>
                        <select name="searchby" class="form-control">
                            <option value="created_at" <?=!empty($_GET['searchby'])?(($_GET['searchby'] =='created_at')?'selected':''):''?> >Lead Created Date</option>
                            <option value="updated_at" <?=!empty($_GET['searchby'])?(($_GET['searchby'] =='updated_at')?'selected':''):''?> >Lead Updated Date</option>
                            <option value="capture_time" <?=!empty($_GET['searchby'])?(($_GET['searchby'] =='capture_time')?'selected':''):''?> >Lead Captured Date</option>
                        </select>
                    </div>
                </div> --}}
                <div class="col-md-4">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody> <?php $srno = 0; ?>
                            @foreach ($leads as $list)
                                <?php
                                    $rem = App\Models\LeadRemainder::select()->where('status', '1')->where('is_delete', '0')->where('user_id', Auth::user()->id)->where('lead_id', $list['id'])->first();
                                    $now = time();
                                    $your_date = strtotime($rem['next_date']);
                                    $datediff = $your_date - $now;

                                    $countdays = round($datediff / (60 * 60 * 24));
                                    $bg = "";
                                    if($now > $your_date){
                                        $bg = "bg-danger";
                                    }else if($countdays < 6){
                                        $bg = "bg-warning";
                                    }else if($countdays < 11){
                                        $bg = "bg-success";
                                    }
                                ?>
                                <tr class="{{$bg}}">
                                    <td><?= ++$srno ?></td>
                                    <td><?= date("M d, Y",strtotime($rem['next_date'])) ?></td>
                                    ...
                                    <td>
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <a href="{{url('/tele-caller/lead-detail/'.Crypt::encrypt($list['id'])).'?update=remark'}}" type="button" class="btn btn-outline-success"><i class="fa fa-eye"></i></a>
                                            <a href="{{url('/tele-caller/lead-edit/'.Crypt::encrypt($list['id']))}}" type="button" class="btn btn-outline-primary"><i class="fa fa-edit"></i></a>
                                    ...
                                    <td>
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <a onclick="deletemdl('{{ Crypt::encrypt($list['id']) }}')"
                                                type="button" title="Remove From Schedule" class="btn btn-block btn-outline-danger"><i
  ...
  <div class="modal fade" id="deletemdl" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
        <div class="modal-content">
            <form method="POST" action="{{ asset('/tele-caller/schedule/delete') }}">
                @csrf
                ...
                <div class="modal-body justify-content-center flex-column d-flex">
                    ...
                    <p class="mt-4 fs-5 text-center">You can only removed this from Schedule List</p>
                </div>
